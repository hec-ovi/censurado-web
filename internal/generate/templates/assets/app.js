...
// same order (X, WhatsApp, Telegram, LinkedIn, Facebook): the CSS class, the
// aria-label, the brand-glyph SVG path, and the share href built from the
// url-encoded title (t) and the article's absolute URL (u). Kept in sync with
// article_card.tmpl so client-rebuilt cards carry the identical share row.
...
// shareRow builds the .card-share brand link row the server card_inner renders,
// so a scroll-loaded or live-refreshed card carries the same share links as a
// server-rendered one. The share target is the article's ABSOLUTE permalink
// (absURL over the root-relative e.url, matching the server's .Canonical); the
// title and url are encodeURIComponent-escaped. SVG/path nodes are built with
// createElementNS so they land in the SVG namespace; no innerHTML.
function shareRow(e) {
  const share = el("div", { class: "card-share" });
  const t = encodeURIComponent(e.title || "");
  const u = encodeURIComponent(absURL(e.url));
  for (const net of SHARE_NETWORKS) {
    const link = el("a", {
      class: "card-share-link " + net.cls,
      href: net.href(t, u),
      target: "_blank",
    ...
    svg.setAttribute("class", "card-share-icon");
    svg.setAttribute("viewBox", "0 0 24 24");
    svg.setAttribute("aria-hidden", "true");
    svg.setAttribute("focusable", "false");
    const path = document.createElementNS(SVG_NS, "path");
    path.setAttribute("d", net.path);
...
function articleItemFromEntry(e, helpers) {
  ...
  // The brand share row, last in the body (server card_inner order), with the
  // article's absolute permalink as the share target.
  body.appendChild(shareRow(e));
